...
double eval_dubiner_basis_2d(double in_r, double in_s, int in_mode, int in_basis_order)
{
    ...
    {
        std::cout << "ERROR: Invalid mode when evaluating Dubiner basis ...." << std::endl;
    }
...
double eval_dr_dubiner_basis_2d(double in_r, double in_s, int in_mode, int in_basis_order)
{
    ...
    {
        std::cout << "ERROR: Invalid mode when evaluating basis ...." << std::endl;
    }
...
double compute_eta(int vcjh_scheme, int order)
{
    ...
    if (order == 0 && vcjh_scheme != 1)
        ...
        {
            std::cout << "ERROR: Invalid VCJH scheme ... " << std::endl;
        }
...
int index_locate_int(int value, int* array, int size)
{
    ...
    {
        std::cout << "ERROR, array not sorted, exiting" << std::endl;
...
array<T> multPoly(array<T>& p1, array<T>& p2)
{
    ...
    else
    {
        int lengthp1 = p1.get_dim(1);
        int heightp1 = p1.get_dim(0);
        //std::cout<<"length p1: "<<lengthp1<<"   height p1: "<<heightp1<<std::endl;
        int lengthp2 = p2.get_dim(1);
        int heightp2 = p2.get_dim(0);
        //std::cout<<"length p2: "<<lengthp2<<"   height p2: "<<heightp2<<std::endl;
...
double evalPoly(array<double> p, array<double> coords)
{
    ...
    for (int k = 0; k < depthp; k++)
    {
        ...
        for (int i = 0; i < heightp; i++)
        {
            ...
            val *= rowval;
            //std::cout<<"k = "<<k<<" ;  rowval = "<<rowval<<" ;  val = "<<val<<" ;  total = "<<total<<std::endl;
        }
...
void gaussj(int n, array<double>& A, array<double>& b)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        indxc(i) = icol;
        if (A(icol, icol) == 0.0) {
            std::cout << "Error: Singular matrix in gaussj" << std::endl;
            exit(1);
        }
...
array <double> mult_arrays(array <double>& M1, array <double>& M2)
{
    ...
    if (dim_1_2 == 1 and dim_1_3 == 1 and dim_2_2 == 1 and dim_2_3 == 1) {
        ...
        if (dim_1_1 == dim_2_0) {
            ...
            std::cout << "ERROR: array dimensions are not compatible in multiplication function" << std::endl;
            exit(1);
        }
    }
    else {
        std::cout << "ERROR: Array multiplication function can only multiply 2-dimensional arrays together" << std::endl;
        exit(1);
    }
}
...
array <double> transpose_array(array <double>& in_array)
{
    ...
    if (dim_2 == 1 and dim_3 == 1 and dim_0 == dim_1) {
        ...
        std::cout << "ERROR: Array transpose function only accepts a 2-dimensional square array" << std::endl;
        exit(1);
    }
}
...
array <double> inv_array(array <double>& in_array)
{
    ...
    if (dim_2 == 1 and dim_3 == 1) {
        if (dim_0 == dim_1)
        {
            ...
            std::cout << "ERROR: Can only obtain inverse of a square array" << std::endl;
            exit(1);
        }
    }
    else {
        std::cout << "ERROR: Array you are trying to invert has > 2 dimensions" << std::endl;
        exit(1);
    }
}
